...
<head>
    <meta charset="UTF-8">
    <title>Edit Student: {{ $student->full_name }}</title>
    <script defer src="https://unpkg.com/alpinejs@3.x.x/dist/cdn.min.js"></script>
</head>
<body style="font-family: system-ui, sans-serif; padding: 20px; max-width: 800px; margin: 0 auto; color: #333;">

    <h1>Edit Student Details: {{ $student->full_name }}</h1>
    <a href="{{ route('admin.students.show', $student->id) }}"><button>&larr; Back to Profile</button></a>
    <hr>
    ...
    {{-- Safe Data Bridge to inject relational elements avoiding duplication loops --}}
    <script>
        window.__studentEmergencyContacts = @json($student->emergencyContacts ?? []);
    </script>

    <form action="{{ route('admin.students.update', $student->id) }}" 
          method="POST" 
          x-data="studentForm(window.__studentEmergencyContacts)">
        @csrf 
        @method('PUT')
        ...
        <div style="display: flex; gap: 15px; margin-bottom: 10px;">
            <div style="flex: 1;">
                ...
                <input type="text" name="first_name" value="{{ old('first_name', $student->first_name) }}" required style="width: 100%; box-sizing: border-box; padding: 8px;">
            </div>
            <div style="flex: 1;">
                <label style="font-weight:bold;">Last Name:</label>
                <input type="text" name="last_name" value="{{ old('last_name', $student->last_name) }}" required style="width: 100%; box-sizing: border-box; padding: 8px;">
            </div>
        </div>
        ...
        <div style="display: flex; gap: 15px; margin-bottom: 10px;">
            <div style="flex: 1;">
                ...
                <input type="email" name="email" value="{{ old('email', $student->email) }}" required style="width: 100%; box-sizing: border-box; padding: 8px;">
            </div>
            <div style="flex: 1;">
                <label style="font-weight:bold;">Phone Number:</label>
                <input type="text" name="phone" value="{{ old('phone', $student->phone) }}" style="width: 100%; box-sizing: border-box; padding: 8px;">
            </div>
        </div>
        ...
        <div style="display: flex; gap: 15px; margin-bottom: 10px;">
            <div style="flex: 1;">
                ...
                <input type="date" name="details[date_of_birth]" value="{{ old('details.date_of_birth', $student->details->date_of_birth ? $student->details->date_of_birth->format('Y-m-d') : '') }}" style="width: 100%; box-sizing: border-box; padding: 8px;">
            </div>
            <div style="flex: 1;">
                <label style="font-weight:bold;">Gender:</label>
                <select name="details[gender]" style="width: 100%; padding: 8px;">
                    <option value="">-- Select Gender --</option>
                    <option value="male" {{ old('details.gender', $student->details->gender ?? '') == 'male' ? 'selected' : '' }}>Male</option>
                    <option value="female" {{ old('details.gender', $student->details->gender ?? '') == 'female' ? 'selected' : '' }}>Female</option>
                    <option value="other" {{ old('details.gender', $student->details->gender ?? '') == 'other' ? 'selected' : '' }}>Other</option>
                    <option value="prefer_not_to_say" {{ old('details.gender', $student->details->gender ?? '') == 'prefer_not_to_say' ? 'selected' : '' }}>Prefer Not to Say</option>
                </select>
            </div>
        </div>
        ...
        <div style="margin-bottom: 10px;">
            ...
            <input type="text" name="details[national_id_or_passport]" value="{{ old('details.national_id_or_passport', $student->details->national_id_or_passport ?? '') }}" style="width: 100%; box-sizing: border-box; padding: 8px;">
        </div>

        <div style="margin-bottom: 10px;">
            <label style="font-weight:bold;">Home Address:</label>
            <textarea name="details[address]" style="width: 100%; height: 60px; box-sizing: border-box; padding: 8px;">{{ old('details.address', $student->details->address ?? '') }}</textarea>
        </div>

        <div style="margin-bottom: 15px;">
            <label style="font-weight:bold;">Academic Credentials / Background Details:</label>
            <textarea name="details[academic_background]" style="width: 100%; height: 80px; box-sizing: border-box; padding: 8px;">{{ old('details.academic_background', $student->details->academic_background ?? '') }}</textarea>
        </div>
